...
func enter_state()->void:
	var target: Node2D = state_machine.user.interact_target
	if target is Interactive:
		for pos in target.occupied_positions:
			...
			if range_factor <= 1:
				state_machine.user.interact_target.call_deferred("_interacted", state_machine.user)
				break
	else:
		...
		if range_factor <= 1:
			state_machine.user.interact_target.call_deferred("_interacted", state_machine.user)
	state_machine.user.interact_target = null
